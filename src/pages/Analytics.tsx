          ...
          <div className="flex items-center">
            {([60, 180, 600] as const).map((r) => (
              <button
                ...
                // `min-h-8`: these were 23px tall, which is a small target for the
                // one control on this page that a phone user has to reach for.
                className={`readout min-h-8 px-2.5 py-1 text-[10px] transition-colors ${
              ...
              <li key={e.id} className="flex items-start gap-2.5 px-3 py-[6px]">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex flex-wrap items-baseline gap-x-2">
                    ...
                    <span className="readout text-[9px] text-ink-dim">{formatClock(e.timestamp)}</span>
                    <span className="readout text-[9px] text-ink-dim">{e.state}</span>
      </div>

      {/*
        Provenance, stated before the charts rather than in a footnote.

        These traces are generated by the same in-browser simulation that drives
        the cockpit, so they demonstrate that the estimator behaves correctly —
        they are not a measurement of anything. A judge should not have to infer
        that from a small label, and nothing on this page should be mistaken for
        a result from a real dataset.
      */}
      <div className="flex items-start gap-2 border-b border-hairline bg-surface/60 px-4 py-2.5">
        <Info size={12} className="mt-[2px] shrink-0 text-degraded" />
        <p className="text-[10.5px] leading-relaxed text-ink-dim">
          <strong className="font-semibold text-degraded-soft">Simulated traces.</strong> These
          charts are produced by the in-browser model that drives the cockpit, and no real sensor
          recording is loaded here. They show how the estimator responds to a controlled outage
          sequence; they are not experimental results. Benchmarks against recorded flight or
          vehicle data are tracked with the research write-up, outside this demo.
        </p>
      </div>
  ...
  return (
    ...
    >
      <div className="px-2 pb-1 pt-1">
        {children}
        <p className="px-1 pb-1 pt-1 text-[10px] leading-snug text-ink-dim">{note}</p>
      </div>
    </Panel>
  );
}
